...
class EsiHelper {
	...
	public function getCharacterImageByName($characterName, $imageOnly = true, $size = 128, $newWidth = null, $newHeight = null) {
		...
		if(!\is_null($characterID)) {
			...
			$imagePath = ImageHelper::getInstance()->getLocalCacheImageUriForRemoteImage('character', $this->imageserverUrl . $this->imageserverEndpoints['character'] . $imageName);
	...
	public function getEntityLogoByName($entityName, $entityType, $imageOnly = true, $size = 128) {
		...
		if(!\is_null($eveID)) {
			...
			$imagePath = ImageHelper::getInstance()->getLocalCacheImageUriForRemoteImage($entityType, $this->imageserverUrl . $this->imageserverEndpoints[$entityType] . $imageName);
	...
	private function getEsiData($route) {
		...
		$transientName = \sanitize_title('yulai-federation-theme-esi-data_' . $route);
