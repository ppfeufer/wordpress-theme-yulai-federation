...
class Corppage {
	...
	public function savePageSettings($postID) {
		if(empty($_POST['_yf_corp_page_nonce']) || !\wp_verify_nonce($_POST['_yf_corp_page_nonce'], 'save')) {
			return false;
		} // END if(empty($_POST['_yf_corp_page_nonce']) || !\wp_verify_nonce($_POST['_yf_corp_page_nonce'], 'save'))
		...
		\update_post_meta($postID, 'yf_page_corp_name', $_POST['yf_page_corp_name']);

		$corpID = $this->eveApi->getEveIdFromName(\stripslashes($_POST['yf_page_corp_name']));
		\update_post_meta($postID, 'yf_page_corp_eve_ID', \esc_html($corpID));

		$isCorpPage = \filter_input(INPUT_POST, 'yf_page_is_corp_page') == "on";
		\update_post_meta($postID, 'yf_page_is_corp_page', $isCorpPage);(\stripslashes($_POST['yf_page_corp_name']));
