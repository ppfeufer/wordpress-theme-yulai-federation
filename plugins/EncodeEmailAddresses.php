...
class EncodeEmailAddresses {
	...
	public function encodeMails($content) {
		...
		// abort if `yf-encode-email-address_at-sign-check` is true and `$content` doesn't contain a @-sign
		if(\apply_filters('yf-encode-email-address_at-sign-check', true ) && \strpos($content, '@') === false) {
			return $content;
		} // END if(\apply_filters('yf-encode-email-address_at-sign-check', true ) && \strpos($content, '@') === false)
		...
		$regexp = \apply_filters(
			'yf-encode-email-address_regexp',
			'{
				(?:mailto:)?
				(?:
					[-!#$%&*+/=?^_`.{|}~\w\x80-\xFF]+
